...
import { FlatList, Text, View, StyleSheet, Linking, ScrollView, TouchableOpacity, Image, Alert } from 'react-native'
...
class SideMenu extends Component {
  ...
  getSocialMediaIcon (item, index) {
    return <TouchableOpacity key={index} onPress={() => item.action()}>
  ...
  getMenuByKey (item, index) {
    switch (item.key) {
      case 'פרופיל':
        return <View>
          <LinearGradient colors={['#e94989', '#e31c6c', '#C2185B', '#9f144b']} start={[0.5, 0]} end={[0.75, 0.85]}>
            <TouchableOpacity
    ...
    return (
      <ScrollView>
        ...
        <View>
          ...
          />
          <Text style={styles.textVersionStyle}>{`${Constants.manifest.version} Version`}</Text>
        </View>
      </ScrollView>
    )
  }
